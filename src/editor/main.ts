...
} from '../features/settings/component';
...
async function getEditorSettings(): Promise<EditorSettings> {
  if (!window.b2sDesignerAPI) return defaultEditorSettings;
  const settings = await window.b2sDesignerAPI.getSettings();
  return {
    theme: settings.theme || defaultEditorSettings.theme,
  };
}

async function saveEditorSettings(settings: EditorSettings): Promise<void> {
  if (window.b2sDesignerAPI) {
    await window.b2sDesignerAPI.saveSettings({
      theme: settings.theme as 'system' | 'dark' | 'light',
    });
  }
}
...
function showSettingsModal(): void {
  ...
  getEditorSettings().then(settings => {
    ...
    initSettingsComponent(content, settings, {
      onSave: async newSettings => {
        await saveEditorSettings(newSettings);
...
function showResizeModal(): void {
  ...
  const api = window.b2sDesignerAPI as { openResize?: (w: number, h: number) => void } | undefined;
  if (api?.openResize) {
...
function setupModals(): void {
  ...
  aboutModal?.querySelector('.modal-backdrop')?.addEventListener('click', () => {
...
async function init(): Promise<void> {
  ...
  if (window.b2sDesignerAPI) {
    const appSettings = await window.b2sDesignerAPI.getSettings();
...
async function checkPendingFile(): Promise<void> {
  const api = window.b2sDesignerAPI as unknown as {
...
function setupApiListeners(): void {
  if (window.b2sDesignerAPI) {
    window.b2sDesignerAPI.onFileOpened(async data => {
      await loadFile(data.path, data.content);
    });

    const extApi = window.b2sDesignerAPI as unknown as { onFileReady?: (cb: () => void) => void };
    extApi.onFileReady?.(() => {
      checkPendingFile();
    });

    window.b2sDesignerAPI.onNewFile(() => {
      newFile();
    });

    window.b2sDesignerAPI.onSaveRequested(() => {
      saveFile();
    });

    window.b2sDesignerAPI.onShowSettings?.(() => {
      window.dispatchEvent(new CustomEvent('show-settings-dialog'));
    });

    window.b2sDesignerAPI.onShowAbout?.(() => {
      const modal = document.getElementById('about-modal');
      if (modal) modal.classList.remove('hidden');
    });

    window.b2sDesignerAPI.onSaveBeforeClose?.(() => {
      saveFile().then(() => {
        window.b2sDesignerAPI.closeAfterSave?.();
      });
    });

    (
      window.b2sDesignerAPI as {
    ...
    (
      window.b2sDesignerAPI as { onAnimationDeleted?: (cb: (data: { name: string }) => void) => void }
    ).onAnimationDeleted?.(data => {
      handleAnimationDeleted(data.name);
    });

    (window.b2sDesignerAPI as { onResizeResult?: (cb: (settings: ResizeSettings) => void) => void }).onResizeResult?.(
      settings => {
        performResize(settings);
      }
    );

    (window.b2sDesignerAPI as { onMenuAction?: (cb: (action: string) => void) => void }).onMenuAction?.(action => {
      handleMenuAction(action);
    });

    (window.b2sDesignerAPI as { onSetInputDisabled?: (cb: (disabled: boolean) => void) => void }).onSetInputDisabled?.(
      disabled => {
  ...
  window.addEventListener('open-animation-editor', ((e: CustomEvent) => {
    ...
    const api = window.b2sDesignerAPI as unknown as { openAnimationEditor?: (a: unknown, n: boolean) => void };
...
function toggleScoreFrames(): void {
  ...
  if (window.b2sDesignerAPI) {
    window.b2sDesignerAPI.saveSettings({ showScoreFrames: state.showScoreFrames });
  }
  render();
}
...
async function saveFile(): Promise<void> {
  if (!window.b2sDesignerAPI) return;

  const content = writeDirectB2S(state.currentData);

  if (state.currentFilePath) {
    const success = await window.b2sDesignerAPI.saveFile(state.currentFilePath, content);
    if (success) {
      markClean();
      undoManager.setSavePoint();
    ...
    const result = await window.b2sDesignerAPI.saveFileAs(content);
    if (result) {
