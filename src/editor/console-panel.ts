...
function showConsole(): void {
  ...
  window.b2sDesignerAPI?.saveConsoleSettings?.({ visible: true });
...
function hideConsole(): void {
  ...
  window.b2sDesignerAPI?.saveConsoleSettings?.({ visible: false });
  setTimeout(resizeCanvas, 0);
}
...
const onConsolePointerEnd = (e: PointerEvent): void => {
  ...
  if (consolePanel) {
    window.b2sDesignerAPI?.saveConsoleSettings?.({ height: consolePanel.offsetHeight });
  }
};
...
async function initConsole(): Promise<void> {
  ...
  if (api?.getConsoleSettings) {
    const settings = await api.getConsoleSettings();
