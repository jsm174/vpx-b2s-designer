...
function writeImages(lines: string[], images: DirectB2SData['images']): void {
  ...
  if (images.backgroundImages.length > 0) {
    const img = images.backgroundImages[0];
    lines.push(`    <BackglassImage Value="${img.imageData}" FileName="${escapeXml(img.fileName)}"/>`);
  }

  if (images.dmdImages.length > 0) {
    const img = images.dmdImages[0];
    lines.push(`    <DMDImage Value="${img.imageData}" FileName="${escapeXml(img.fileName)}"/>`);
  }

  if (images.thumbnailImage) {
    lines.push(`    <ThumbnailImage Value="${images.thumbnailImage}"/>`);
  }
