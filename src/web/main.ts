import type { B2SDesignerAPI, AppSettings, ConsoleSettings } from '../types/api';
...
const api: B2SDesignerAPI = {
  ...
  async getSettings(): Promise<AppSettings> {
    const stored = localStorage.getItem('b2s-designer-settings');
    if (stored) {
      try {
  ...
  async saveSettings(settings: Partial<AppSettings>): Promise<void> {
    ...
    localStorage.setItem('b2s-designer-settings', JSON.stringify({ ...current, ...settings }));
  },

  async getConsoleSettings(): Promise<ConsoleSettings | null> {
    const stored = localStorage.getItem('b2s-console-settings');
    if (stored) {
      try {
  ...
  async saveConsoleSettings(settings: ConsoleSettings): Promise<void> {
    ...
    localStorage.setItem('b2s-console-settings', JSON.stringify({ ...current, ...settings }));
  },
...
(window as unknown as { b2sDesignerAPI: B2SDesignerAPI }).b2sDesignerAPI = api;
...
async function getEditorSettings(): Promise<EditorSettings> {
  const stored = localStorage.getItem('b2s-editor-settings');
  if (stored) {
    try {
...
async function saveEditorSettings(settings: EditorSettings): Promise<void> {
  localStorage.setItem('b2s-editor-settings', JSON.stringify(settings));
}
...
function setupModals(): void {
  ...
  aboutModal?.querySelector('.modal-backdrop')?.addEventListener('click', hideAboutModal);
